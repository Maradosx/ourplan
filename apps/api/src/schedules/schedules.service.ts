...
export class SchedulesService {
    ...
    const expanded = recurringBase
      ...
      .map((sc) => {
        ...
        const newStart = new Date(
          `${date}T${this.tp(sc.startDatetime.getHours())}:${this.tp(sc.startDatetime.getMinutes())}:00`,
        );
        return {
          ...sc,
          startDatetime: newStart,
          endDatetime: new Date(newStart.getTime() + dur),
        };
      });
        ...
        if (this.doesRecurOnDate(sc, dateObj)) {
          expanded.push({
            ...
            startDatetime: new Date(
              ...
              month,
              d,
              sc.startDatetime.getHours(),
              sc.startDatetime.getMinutes(),
            ),
          });
        }
      }
  ...
  async findDayAllFriends(userId: string, date: string, filterIds?: string[]) {
    ...
    const all = [
      ...fRegular,
      ...expandRecurring(fRecurring),
      ...myRegular,
      ...expandRecurring(myRecurring),
    ].sort((a, b) => a.startDatetime.getTime() - b.startDatetime.getTime());

    return all.map((e) => ({
      ...e,
      friendDisplayName:
        e.userId === userId ? null : (friendMap[e.userId]?.displayName ?? ''),
      friendAvatarUrl:
        e.userId === userId ? null : (friendMap[e.userId]?.avatarUrl ?? null),
  ...
  async findMonthAllFriends(
    ...
    if (filterIds && filterIds.length > 0)
      friendIds = friendIds.filter((id) => filterIds.includes(id));
    ...
    return [
      ...fRegular,
      ...expandMonth(fRecurring),
      ...myRegular,
      ...expandMonth(myRecurring),
    ].sort((a, b) => a.startDatetime.getTime() - b.startDatetime.getTime());
  }
  ...
  async findFreeSlots(
    ...
    if (friendIds && friendIds.length > 0)
      friends = friends.filter((f) => friendIds.includes(f.id));

    const participants = [
      { id: userId, displayName: 'Me', avatarUrl: null },
      ...friends,
    ];
    ...
    const schedules = await this.prisma.schedule.findMany({
      where: {
        ...
        OR: [
          ...
          {
            userId: { in: friendParticipantIds },
            visibility: { not: 'private' },
          },
        ],
      },
    ...
    for (let d = 0; d < days; d++) {
      ...
      const checkInterval = (slotStart: Date, slotEnd: Date) => {
        ...
        for (const id of participantIds) {
          const isBusy = busyByUser[id].some(
            (b) => b.s < slotEnd && b.e > slotStart,
          );
          if (!isBusy) freeCount++;
        }
        return freeCount;
  ...
  private doesRecurOnDate(
    sc: { startDatetime: Date; recurrenceRule: string | null },
    date: Date,
  ): boolean {
    if (!sc.recurrenceRule) return false;
    try {
      const rule = JSON.parse(sc.recurrenceRule) as {
        type: string;
        weekDays?: number[];
        endDate?: string;
      };
      ...
      if (rule.endDate && date > new Date(rule.endDate + 'T23:59:59'))
        return false;
      switch (rule.type) {
        case 'daily':
          return true;
        case 'weekly':
          return (rule.weekDays ?? []).includes((date.getDay() + 6) % 7);
        case 'monthly':
          return date.getDate() === orig.getDate();
        case 'yearly':
          return (
            date.getMonth() === orig.getMonth() &&
            date.getDate() === orig.getDate()
          );
        default:
          return false;
      }

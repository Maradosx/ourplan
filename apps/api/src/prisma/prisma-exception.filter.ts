  ...
  private readonly logger = new Logger(PrismaExceptionFilter.name);
    ...
    switch (exception.code) {
      case 'P2025': // Record to update/delete does not exist
        httpException = new NotFoundException('The requested resource was not found.');
        break;
      case 'P2002': {
        // Unique constraint failed
        const target = (exception.meta?.target as string[] | undefined)?.join(', ');
        httpException = new ConflictException(
          target ? `A record with this ${target} already exists.` : 'Duplicate record.',
        );
        break;
      }
      case 'P2003': // Foreign key constraint failed
        httpException = new BadRequestException('Related resource does not exist.');
        break;
      default:
